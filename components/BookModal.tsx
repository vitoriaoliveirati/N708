...
export default function BookModal({
  ...
  // Se não estiver aberto ou não tiver livro, não renderiza modal
  if (!isOpen || !book) return null;

  // Token do usuário
  const token = typeof window !== 'undefined'
    ? localStorage.getItem('biblio_token')
    : null;

  // -----------------------------
  // 📌 FUNÇÃO: Reservar o livro
  // -----------------------------
  async function reservar() {
    ...
    const res = await fetch('/api/reservas', {
      ...
      body: JSON.stringify({ livroId: book!.id }) // <- Garantido não-null
    });
      ...
      <div className="bg-white rounded p-6 max-w-xl w-full">
        ...
        <div className="flex gap-4">
          ...
          <div>
            ...
            <div className="mt-4">
              {book!.disponivel ? (
                <span className="px-3 py-1 bg-green-200 rounded">Disponível</span>
              ) : (
                <span className="px-3 py-1 bg-red-200 rounded">Indisponível</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <button
            onClick={reservar}
            disabled={loading || !book!.disponivel}
            className="px-4 py-2 bg-blue-500 text-white rounded"
          >
            {book!.disponivel ? 'Reservar' : 'Indisponível'}
          </button>
        </div>
